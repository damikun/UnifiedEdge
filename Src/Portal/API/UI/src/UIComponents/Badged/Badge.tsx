...
const Badge_SIZE = {
  ...
  nano: "h-4 text-xs md:text-xs",
  thin: "h-5 text-xs md:text-xs",
...
export type BadgeProps = {
  ...
  rounding?: keyof typeof Badge_ROUNDING;
  className?: string;
...
function Badge({
  ...
  const Var_Style = Badge_VARIANTS[variant] || Badge_VARIANTS.clean;
  const Var_Rounding = Badge_ROUNDING[rounding] || Badge_ROUNDING.large;
  const Var_Size = Badge_SIZE[size] || Badge_SIZE.auto;
